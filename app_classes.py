...
class Csv:
    ...
    def get_file(self):
        """
        Prompt the user to select a CSV file and return its path.

        Returns:
            str or None: The file path of the selected CSV file or None if the selection is cancelled.
        """
        root = Tk()
        ...
        if not self.file_path:  # File selection was cancelled
            print("File selection cancelled.")
            return False, "File selection cancelled."
        if not self.file_path.lower().endswith(".csv"):
            print("Selected file is not a CSV.")
            return False, "Selected file is not a CSV."

        print("Selected file:", self.file_path)
        return True, self.file_path

    def get_target(self):
        # Använd koden uner när du inte testar koden
        ...
        if target not in targets:
            print("You didn't choose a column to predict\n")
            return None
        # target = "sales"
        return target
